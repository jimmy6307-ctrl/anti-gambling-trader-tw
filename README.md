...
# 6. 想用 Excel？產生範本；最簡單只填代號、平倉日、損益、策略
...
3. **完整階段連動**：不是只看樣本內裁決；只有樣本外延續、幣別與風險基準都可靠，
   且分流結果為 `tiny_live_validation` 時，設定檔才可能顯示 live 可用。其餘情況一律 `false`。
...
> 339 個自動化測試。歡迎任何人檢驗與挑戰 —— 這正是本工具要求「老師們」
> 做到的事。
